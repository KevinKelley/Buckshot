...
abstract class HtmlSurfaceElement implements BoxModelSurfaceElement
{
  ...
  static void _startWatchMeasurement(HtmlSurfaceElement element){
    void watchIt(num _){
      htmlPresenter.measure(element).then((RectMeasurement m){
    ...
    htmlPresenter.workers['${element.safeName}_watch_measurement'] = watchIt;
  }

  static void _stopWatchMeasurement(HtmlSurfaceElement element){
    if (htmlPresenter.workers.containsKey('${element.safeName}_watch_measurement')){
      htmlPresenter.workers.remove('${element.safeName}_watch_measurement');
    }

    _previousMeasurement[element] = null;
  }

  static void _startWatchPosition(HtmlSurfaceElement element){
    void watchIt(num _){
      htmlPresenter.measure(element).then((RectMeasurement m){
    ...
    htmlPresenter.workers['${element.safeName}_watch_position'] = watchIt;
  }

  static void _stopWatchPosition(HtmlSurfaceElement element){
    if (htmlPresenter.workers.containsKey('${element.safeName}_watch_position')){
      htmlPresenter.workers.remove('${element.safeName}_watch_position');
    }
  ...
  /**
   * Sets event handlers for base HtmlSurfaceElement events.
   */
  static void initializeBaseEvents(HtmlSurfaceElement element){
    ...
    void mouseLeaveHandler(e){
      ...
      htmlPresenter.measure(element).then((RectMeasurement r){
        localMouseCoordinate(element, e.pageX, e.pageY)
          .then((p){
  ...
  static void updateChildAlignment(FrameworkContainer element){
    ...
    assert(element.containerContent is HtmlSurfaceElement);
   ...
   * given [brush].
   */
  static void setBackgroundBrush(HtmlSurfaceElement element, Brush brush){
   ...
   * coordinate space.
   */
  static Future<SurfacePoint> localMouseCoordinate(HtmlSurfaceElement element,
      num pageX, num pageY){
